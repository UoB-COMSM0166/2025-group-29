...
//   if (powerMode && millis() - powerModeStartTime > 10000) {
//     powerMode = false;
//   }

//   if (slowEffect && millis() - slowEffectStartTime > 15000) {
//     slowEffect = false;
//   }
...
//     if (!isInvincible && enemies[i].eats(player)) {
//       gameOver = true;
//       return;
//     }
//   }
  
//     if (isSlowed && millis() > slowEndTime) {
//     isSlowed = false;
//   }
...
//     textSize(20);
//     textAlign(CENTER);
//     text(warningMessage, player.pos.x, player.pos.y - 50);
//   }
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    if (player.eats(enemies[i])) {
      ...
      enemies.splice(i, 1);
    } else if (enemies[i].eats(player) && !powerMode) {
      gameOver = true;
      return;
    }
  }
...
class Player {
  ...
  update() {
    ...
    mouseVec.setMag(this.speed); //使用动态速度
    this.pos.add(mouseVec);
    // let speed = isSlowed ? 2 : 4;
