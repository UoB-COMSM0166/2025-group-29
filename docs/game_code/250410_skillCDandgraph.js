...
function preload() {
  ...
  skillSystem.addSkill(new Skill("反弹", "F", "demo_skill.png", 10));
  skillSystem.addSkill(new Skill("护盾", "G", "demo_skill.png", 8));
  skillSystem.addSkill(new Skill("加速", "H", "demo_skill.png", 12));
...
class SkillSystem {
  ...
  draw() {
    let size = 64;
    let spacing = 10;
    let totalWidth = this.skills.length * (size + spacing) - spacing;
  ...
  tryActivateSkill(keyPressed) {
    for (let skill of this.skills) {
      if (keyPressed.toUpperCase() === skill.key && skill.cooldownRemaining <= 0) {
        skill.trigger();
      }
    }
  }
