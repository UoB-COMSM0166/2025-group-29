...
// —— 新增：解析 URL 参数，读取 ?save=slotName —— 
function getSaveParam() {
  const params = new URLSearchParams(window.location.search);
  return params.get('save');
}
function loadSavedGame(slotName) {
  if (!slotName) return;
  const raw = localStorage.getItem(`save_${slotName}`);
  if (!raw) {
    console.warn(`No save found: save_${slotName}`);
    return;
  }
  try {
    ...
    // 恢复必要的全局变量
    gamelevel  = data.difficulty;
    // 如果需要把 currentLevel、timer 等也恢复，可以在这里赋值
    // 例如： startTime = millis() - 已用时；score = data.score;
  } catch (e) {
    console.error('Failed to parse save data:', e);
  }
}

let slotName = getSaveParam();
